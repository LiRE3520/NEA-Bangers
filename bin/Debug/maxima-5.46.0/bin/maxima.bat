...
/tmp/maxima-5.46.0/crosscompile-windows/build64/wine-clisp.sh %lisp_options% -q -M "%maxima_image_base%.mem" "" -- %*
goto :EOF

:clisp_autotools
rem ./configure sets a variable /tmp/maxima-5.46.0/crosscompile-windows/build64/wine-clisp.sh which might be used instead of lisp.exe,
rem but when Maxima is crosscompiled on Linux, a shell script is used as 'crosscompiler',
rem which would be inserted here ==> do not use the variable /tmp/maxima-5.46.0/crosscompile-windows/build64/wine-clisp.sh but just lisp.exe.
...
rem ./configure sets a variable /tmp/maxima-5.46.0/crosscompile-windows/build64/wine-sbcl.sh which might be used instead of sbcl.exe,
rem but when Maxima is crosscompiled on Linux, a shell script is used as 'crosscompiler',
rem which would be inserted here ==> do not use the variable /tmp/maxima-5.46.0/crosscompile-windows/build64/wine-sbcl.sh but just sbcl.exe.
